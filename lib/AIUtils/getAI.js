...
// 运行时记录已确认不支持图片输入的渠道（进程重启清空）
const noVisionChannels = new Set()

const GLOBAL_SYSTEM_RULES = "\n[重要规则] 禁止在回复中嵌入任何外部图片URL（包括pollinations.ai等）。如果需要生成图片，必须通过 generateImage 工具调用。"

function isImageNotSupportedError(err) {
  if (!err || err.status !== 400) return false
  const haystack = `${err.message || ""} ${JSON.stringify(err.error || {})}`
  return /image_url|image|vision|multimodal|不支持图片/i.test(haystack)
}
...
async function _getOpenAIResponse(
  ...
  try {
    ...
    if (fullSystemInstructionText) {
      fullSystemInstructionText += GLOBAL_SYSTEM_RULES
    ...
    if (historyContents.length > 0) {
      for (const item of historyContents) {
        if (item.role === "user" || item.role === "model") {
          ...
          const reasoningContent = role === "assistant" ? item.reasoning_content : undefined
          let message = { role }
          if (content) message.content = content
          if (tool_calls.length > 0) message.tool_calls = tool_calls
          if (typeof reasoningContent !== "undefined") {
    ...
    if (queryParts && queryParts.length > 0) {
      const stripImages =
        channel.vision === false || noVisionChannels.has(channel.name)
      const processedParts = processQueryParts(queryParts, "openai")
      const openAICompatibleParts = processedParts
        .map(part => {
        ...
        .filter(p => !stripImages || p.type !== "image_url")

      // 纯文本用字符串格式（兼容 DashScope 等不支持数组格式 content 的 API）
      if (openAICompatibleParts.length === 1 && openAICompatibleParts[0].type === "text") {
        messages.push({ role: "user", content: openAICompatibleParts[0].text })
      } else {
    ...
    if (enableTools && toolsSchema && toolsSchema.length > 0) {
      ...
      requestPayload.tools = adjustedSchema.map(tool => ({ type: "function", function: tool }))
      requestPayload.tool_choice = "auto"
    }

    logger.info(`[getAI] 发送请求到 ${channel.baseURL}/${channel.model}，messages: ${messages.length} 条，tools: ${requestPayload.tools ? requestPayload.tools.length : 0} 个`)
    for (let idx = 0; idx < messages.length; idx++) {
      const m = messages[idx]
      const contentPreview = typeof m.content === "string"
        ? m.content.slice(0, 120)
        : Array.isArray(m.content)
          ? m.content.map(c => c.text || "[img]").join("").slice(0, 120)
          : ""
      const tcInfo = m.tool_calls?.length ? ` tool_calls:[${m.tool_calls.map(t => t.function?.name).join(",")}]` : ""
      logger.info(`[getAI] msg[${idx}] role=${m.role}${m.name ? ` name=${m.name}` : ""}${tcInfo} content=${contentPreview || "(empty)"}`)
    }
    ...
    logger.info(`[getAI] 响应耗时 ${elapsed}s，text: ${extractedText.length} 字，toolCalls: ${toolCallsArr.length} 个${toolCallsArr.length > 0 ? " [" + toolCallsArr.map(tc => tc.function?.name).join(", ") + "]" : ""}`)
    const hasReasoningContent =
      !!message && Object.prototype.hasOwnProperty.call(message, "reasoning_content")
    ...
    if (isImageNotSupportedError(error) && !noVisionChannels.has(channel.name)) {
      noVisionChannels.add(channel.name)
      logger.warn(`渠道 [${channel.name}] 不支持图片输入，剥离图片后重试`)
      return _getOpenAIResponse(
        channel,
        e,
        queryParts,
        presetPrompt,
...
async function _getGeminiResponse(
  ...
  try {
    ...
    if (fullSystemInstructionText.trim()) {
      requestConfig.systemInstruction = (fullSystemInstructionText + GLOBAL_SYSTEM_RULES).trim()
    }
