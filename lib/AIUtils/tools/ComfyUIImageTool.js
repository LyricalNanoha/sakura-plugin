...
export class ComfyUIImageTool {
  ...
  func = async (opts, e) => {
    const { tags, style, artist, includeSelf = true, orientation, hasReferenceImage = false, isNsfw = false } = opts
    ...
    // 智能角色标签注入：仅当 includeSelf=true 且 tags 中没有其他角色标签时才加
    let characterTags = ""
    if (includeSelf && config.characterTags) {
      // 检测 tags 中是否包含 Danbooru 角色格式 name_(series) 或明显的其他角色名
      const selfName = (config.characterTags || "").split(",")[0]?.trim().toLowerCase()
      const tagsLower = tags.toLowerCase()
      const hasSeriesTag = /\w+_?\([\w:!_ ]+\)/i.test(tags)
      const hasSelfTag = selfName && tagsLower.includes(selfName)
      if (!hasSeriesTag || hasSelfTag) {
