...
export class ComfyUIImageTool {
  ...
  func = async (opts, e) => {
    const { tags, count = 1, style, artist, includeSelf = false, orientation, hasReferenceImage = false, isNsfw = false } = opts
    ...
    logger.info(`[ComfyUIImage] 正在生成 ${imageCount} 张图片 (workflow: ${workflowName})...`)
    ...
    for (let i = 0; i < imageCount; i++) {
      generateTasks.push(
        generateImage({ positive, negative, width, height, style, referenceImage: referenceImageBuffer })
      )
    }
    ...
    // 保存 prompt 到 Redis，供引用重生使用
    const promptKey = `img_prompt:${e.group_id || e.user_id}:last`
    const promptData = JSON.stringify({ positive, negative, workflowName, width, height, style, artist: artist || "" })
    try {
      await redis.set(promptKey, promptData, { EX: 86400 * 7 })
    } catch {}
