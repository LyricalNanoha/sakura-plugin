...
export async function buildGroupPrompt(groupId, options = {}) {
  ...
  if (chats && chats.length > 0) {
    ...
    const chatContent = formattedChats.join("\n")
    systemPromptWithContext += chatContent
    logger.info(`[GroupContext] 群${groupId} 加载了${chats.length}条消息 (${chatContent.length}字):\n${chatContent.slice(-600)}`)
  }
