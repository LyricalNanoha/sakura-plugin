...
export async function buildGroupPrompt(groupId, options = {}) {
  ...
  let chats = []
  try {
    chats = await getChatHistoryGroup(group, groupContextLength + 10)
    // 过滤掉 bot 自身的消息，避免之前的生图回复污染新请求的上下文
    const botUin = String(Bot.uin)
    chats = chats
      .filter(chat => String(chat.sender?.user_id) !== botUin)
      .slice(-groupContextLength)
  } catch (historyError) {}
