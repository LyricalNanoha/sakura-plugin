...
export const TOOL_DESCRIPTION = "生成图片。支持动漫二次元(anima)和写实真人(krea2)两种风格。当聊天中有生图意图时使用。如果用户发送/引用了参考图片或@了某人（用其头像作参考），设置 hasReferenceImage=true 使用风格参考模式。支持换脸功能：用户要求「换脸」「用我的脸」「把脸换成xx」时设置 faceSwap=true，需要用户提供人脸源图（发送图片、引用图片或@某人取头像）。"
...
生图前必须先调用 searchTags 工具查询相关角色/动作的准确标签，再用查到的标签生成 prompt。
...
- faceSwap：是否进行换脸。当用户明确要求「换脸」「把脸换成」「用我的脸」「face swap」或@某人要求将脸替换到生成图中时设为true。
  换脸需要一张人脸源图（用户发送的图片、引用的图片、或@某人的头像）。
  重要：faceSwap=true 时不要同时设置 hasReferenceImage=true！图片用于换脸就不是风格参考。仅当用户明确同时要求「参考这个风格+换脸」时才两个都设true。

重要：生图前必须先调用 searchTags 工具查询角色/动作/概念的准确 Danbooru 标签，用查到的结果生成 prompt，不要凭记忆猜测。
...
- 用户发送图片说"换脸" → faceSwap:true, style:"krea2", tags:"A photorealistic portrait of a person, high quality, detailed face"
- 用户发送图片说"把我的脸换到这张图上" → faceSwap:true, tags:"Same scene and composition as described, maintaining the original style"
- 用户@某人说"用他的脸画个帅哥" → faceSwap:true, style:"krea2", tags:"A handsome young man in a stylish outfit, professional photography, studio lighting"
- 用户发送图片说"用这个人的脸画个动漫角色" → faceSwap:true, style:"krea2", tags:"The whole scene drawn as 2D anime illustration with cel shading. A character with detailed features, vibrant colors, dynamic pose"

【引用生图记录再修改】当用户引用了之前的生图记录（消息中包含 [原始生图参数]），必须：
1. 以原始 prompt 为基础，保留 workflow 和 style 不变
2. 只修改用户明确要求变更的部分（如"奶子再小一点"→ 只改 breast 相关标签）
3. 其余所有标签和描述必须100%原样复用，不得重新编写或省略
4. 示例：原始prompt="1girl, solo, ayachi nene, large breasts"，用户说"小一点" → 改为"1girl, solo, ayachi nene, small breasts"
