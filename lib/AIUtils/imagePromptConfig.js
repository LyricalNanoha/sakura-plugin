...
export const TAGS_DESCRIPTION = `画面描述，必须使用英文。根据风格使用不同格式：

【anime 风格（无参考图）】使用 Danbooru 标签，英文逗号分隔：
...
【realistic/krea2 风格（无参考图）】使用英文自然语言描述：
- 描述场景、人物、光线、氛围
- 示例: "A photorealistic portrait of a beautiful woman sitting on a park bench, wearing a white dress, soft golden hour lighting, shallow depth of field"
- 示例: "Breathtaking mountain landscape with a crystal clear lake reflecting the sunset, dramatic clouds"
...
/** style 参数描述 */
export const STYLE_DESCRIPTION = `画面风格：
- anime = 动漫二次元（默认）
- realistic = 写实真人照片风格
用户说"写实""真人""照片"→ realistic；说"动漫""二次元""插画"→ anime。
也可直接指定模型名：anima / krea2。`
