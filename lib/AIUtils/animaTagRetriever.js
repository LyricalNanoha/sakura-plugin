...
/**
 * 加载词汇库，构建中文→Danbooru标签的反向索引
 * 支持两种格式：
 * - danbooru.csv: tag_name,category_id,post_count,"aliases",chinese,extra
 * - 提示词汇库.txt: tag_name,id,weight,"aliases,中文"
 */
function loadVocabIndex() {
  if (vocabIndex) return vocabIndex
  try {
    const vocabFile = fs.existsSync(VOCAB_PATH) ? VOCAB_PATH
      : fs.existsSync(VOCAB_LEGACY_PATH) ? VOCAB_LEGACY_PATH : null
    if (!vocabFile) return null

    const raw = fs.readFileSync(vocabFile, "utf-8")
    const lines = raw.split("\n")
    const isNewFormat = vocabFile === VOCAB_PATH
    ...
    for (const line of lines) {
      ...
      if (isNewFormat) {
        // danbooru.csv 格式: tag,category,count,"aliases",chinese,extra
    ...
    logger.info(`[AnimaTagRetriever] 词汇库已加载，${vocabIndex.size} 个中文词条 (${isNewFormat ? "danbooru.csv" : "legacy"})`)
    return vocabIndex
  } catch (err) {
...
export function retrieveTags(userText, options = {}) {
  ...
  // 8. 词汇库补充检索：对结构化数据未覆盖的中文词，从5万条词汇库中查找
  ...
  if (vocab) {
    ...
    for (const [cnKey, entry] of vocab) {
      if (text.includes(cnKey) && !alreadyMatchedTags.has(entry.tag)) {
        result.vocabTags.push({ cn: cnKey, tag: entry.tag, weight: entry.weight })
      }
    }
