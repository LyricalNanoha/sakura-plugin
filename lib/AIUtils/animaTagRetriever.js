...
export function retrieveTags(userText, options = {}) {
  ...
  if (vocab) {
    ...
    for (const [cnKey, entry] of vocab) {
      ...
      if (entry.category === 4) {
        candidateCharacters.push({ cnKey, entry })
        matchedKeys.push(cnKey)
      } else {
        result.vocabTags.push({ cn: cnKey, tag: entry.tag, weight: entry.weight, wiki: entry.wiki })
      }
    }

    // 去重：如果一个短 key 是另一个已匹配长 key 的子串，跳过短的（避免"宁宁"误匹配"绫地宁宁"）
    for (const candidate of candidateCharacters) {
      const { cnKey, entry } = candidate
      const isSubstringOfLongerMatch = matchedKeys.some(
        other => other !== cnKey && other.length > cnKey.length && other.includes(cnKey)
      )
      if (isSubstringOfLongerMatch) continue
