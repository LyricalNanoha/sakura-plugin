...
/**
 * 语义检索增强：当关键词匹配找不到结果时，使用 embedding 相似度搜索
 * @param {string} userText - 用户输入
 * @returns {Promise<Array>} 语义匹配的标签结果
 */
export async function semanticRetrieve(userText) {
  try {
    const { semanticSearch, isIndexReady } = await import("./semanticTagSearch.js")
    if (!isIndexReady()) return []
    const results = await semanticSearch(userText, 5)
    return results
      .filter(r => r.score > 0.35)
      .map(r => ({ name: r.tag, cnName: r.cnName, wiki: r.wiki || "", score: r.score }))
  } catch {
    return []
  }
}
