...
const MAX_HISTORY_ITEMS_TO_SAVE = 10 * 2;

function getConversationKey(e) {
  return e.isGroup ? `${e.group_id}-${e.user_id}` : String(e.user_id);
}
...
export async function loadConversationHistory(e, profilePrefix) {
  ...
  const history = await getHistoryFromRedis(conversationKey, fullRedisPrefix);
