    ...
    for (const msgPart of originalMsg.message) {
      switch (msgPart.type) {
        ...
        case "bface":
          ...
          break
        case "forward": {
          const forwardPrompt = await _extractForwardPrompt(e, msgPart.id || msgPart.resid)
          originalContentParts.push(forwardPrompt || "[聊天记录]")
          break
        }
        case "json":
          try {
            ...
            if (jsonData?.meta?.detail?.resid) {
              const forwardPrompt = await _extractForwardPrompt(e, jsonData.meta.detail.resid)
              originalContentParts.push(forwardPrompt || "[聊天记录]")
            }
          } catch (e) {}
          break
...
async function _extractForwardPrompt(e, resid) {
  if (!resid || !e.group || typeof e.group.getForwardMsg !== "function") return null
  try {
    const forwardMsg = await e.group.getForwardMsg(resid)
    if (!forwardMsg || !Array.isArray(forwardMsg)) return null

    for (const node of forwardMsg) {
      const text = typeof node.message === "string"
        ? node.message
        : Array.isArray(node.message)
          ? node.message.filter(s => s.type === "text").map(s => s.text).join("")
          : null
      if (!text) continue
      const match = text.match(/^\[(K2|Anima|anima_turbo|krea2_style_ref|[^\]]{1,20})\]\s*(.+)/s)
      if (match) {
        return `[原始生图参数] workflow=${match[1]}, prompt="${match[2].trim()}"`
      }
    }
  } catch (err) {
    logger.warn(`[getQuoteContent] 解析转发消息失败: ${err.message}`)
  }
  return null
}
