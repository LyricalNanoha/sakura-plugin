...
/**
 * 在已填充的工作流末尾注入 ReActor 换脸节点（使用 HyperSwap 模型）。
 ...
 * @param {object} config - ComfyUI 配置（用于读取换脸参数）
 */
function injectFaceSwap(workflow, faceImageBase64, config = {}) {
  ...
  if (!wsNodeId || !imageSourceRef) {
    logger.warn("[ComfyUI] 换脸注入失败: 未找到 ETN_SendImageWebSocket 节点")
    return
  }
  ...
  const faceRestoreVisibility = config.faceRestoreVisibility ?? 1.0
  ...
  logger.info(
    `[ComfyUI] ReActor 换脸已注入 (swap_model=${swapModel}, face_restore=${faceRestoreModel}, detection=${faceDetection})`
  )
}
