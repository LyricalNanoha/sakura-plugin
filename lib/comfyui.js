...
export function resolveWorkflow(params) {
  ...
  if (style === "anima" || style === "anime") return "anima_turbo"
...
export function assemblePrompt(params) {
  ...
  if (isKrea2) {
    ...
    const isAnimeIntent = params.style === "k2" || params.style === "krea2"
      ...
      : false
    if (isAnimeIntent && !/anime illustration style/i.test(prompt)) {
      prompt = `2D anime illustration, cel shading, vibrant colors. ${prompt}`
    }
