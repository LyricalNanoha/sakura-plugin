...
export function assemblePrompt(params) {
  ...
  const negativePrompt = config?.negativePrompt || "worst quality, low quality, score_1, score_2, score_3, artist name, blurry, jpeg artifacts, chromatic aberration"
  ...
  if (params.artist) {
    const artist = params.artist.startsWith("@") ? params.artist : `@${params.artist}`
    parts.push(artist)
  }
