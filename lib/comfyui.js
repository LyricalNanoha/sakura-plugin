...
function injectReactor(workflow, faceImageBase64, config = {}) {
  ...
  const baseId = Math.max(...usedIds) + 1

  const swapModel = config.reactorSwapModel || "inswapper_128.onnx"
  const faceRestoreModel = config.reactorFaceRestoreModel || "codeformer-v0.1.0.pth"
  const faceRestoreVisibility = config.reactorFaceRestoreVisibility ?? 1.0

  const loadImageNodeId = String(baseId)
  const reactorNodeId = String(baseId + 1)
  ...
  logger.info(
    `[ComfyUI] ReActor 换脸节点已注入 (swap_model=${swapModel}, face_restore=${faceRestoreModel}, visibility=${faceRestoreVisibility})`
  )
}
