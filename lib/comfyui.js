...
/**
 * 在已填充的工作流末尾注入 FaceFusion 换脸节点。
 * 找到 ETN_SendImageWebSocket 节点，将其图像输入改为 FaceFusion 输出。
 * 使用 AdvancedSwapFaceImage 单节点替换所有人脸，支持 pixel_boost 高分辨率处理。
 * @param {object} workflow - 已填充占位符的工作流对象
 * @param {string} faceImageBase64 - 人脸源图片的 base64 字符串
 * @param {object} config - ComfyUI 配置（用于读取 facefusion 参数）
 */
function injectFaceSwap(workflow, faceImageBase64, config = {}) {
  let wsNodeId = null
  ...
  if (!wsNodeId || !imageSourceRef) {
    logger.warn("[ComfyUI] FaceFusion 注入失败: 未找到 ETN_SendImageWebSocket 节点")
    return
  }
  ...
  const swapperModel = config.ffSwapperModel || "hyperswap_1c_256"
  const detectorModel = config.ffDetectorModel || "scrfd"
  const pixelBoost = config.ffPixelBoost || "512x512"
  const selectorMode = config.ffSelectorMode || "many"
  const maskBlur = config.ffMaskBlur ?? 0.3
  const scoreThreshold = config.ffScoreThreshold ?? 0.3

  const loadImageNodeId = String(nextId++)
  const ffNodeId = String(nextId++)
  ...
  workflow[ffNodeId] = {
    ...
    inputs: {
      source_images: [loadImageNodeId, 0],
      target_image: imageSourceRef,
      api_token: "-1",
      face_swapper_model: swapperModel,
      face_detector_model: detectorModel,
      pixel_boost: pixelBoost,
      face_occluder_model: "xseg_1",
      face_parser_model: "bisenet_resnet_34",
      face_mask_blur: maskBlur,
      face_selector_mode: selectorMode,
      face_position: 0,
  ...
  workflow[wsNodeId].inputs.images = [ffNodeId, 0]

  logger.info(
    `[ComfyUI] FaceFusion 换脸已注入 (model=${swapperModel}, detector=${detectorModel}, pixel_boost=${pixelBoost}, mode=${selectorMode})`
  )
}
    ...
    for (const key of Object.keys(node.inputs)) {
      ...
      switch (value) {
        ...
        case "%reference_image%":
          ...
          break
        case "%face_image%":
          node.inputs[key] = params.faceImage || ""
          break
      }
    }
  }
...
export function resolveWorkflow(params) {
  ...
  if (params.faceImage) return "krea2_face_id"
...
export async function generateImage(params) {
  ...
  const isPulidWorkflow = workflowName === "krea2_face_id"
  if (faceImageBase64 && !isPulidWorkflow) {
    injectFaceSwap(filledWorkflow, faceImageBase64, config)
  }
