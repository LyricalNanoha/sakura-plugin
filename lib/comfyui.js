 ...
 * @param {object} config - ComfyUI 配置（用于读取 reactor 参数）
 */
function injectFaceSwap(workflow, faceImageBase64, config = {}) {
  ...
  const swapModel = config.faceSwapModel || "hyperswap_1c_256.onnx"
  const faceRestoreModel = config.faceRestoreModel || "GFPGANv1.4.pth"
  const faceRestoreVisibility = config.faceRestoreVisibility ?? 1
  const faceDetection = config.faceDetection || "retinaface_resnet50"
  const maxFaces = config.faceMaxFaces || 10
  ...
  const faceIndices = Array.from({ length: maxFaces }, (_, i) => i).join(",")

  workflow[reactorNodeId] = {
    class_type: "ReActorFaceSwap",
    inputs: {
      enabled: true,
      input_image: imageSourceRef,
      ...
      input_faces_index: faceIndices,
      source_faces_index: "0",
      console_log_level: 0,
    },
  }

  workflow[wsNodeId].inputs.images = [reactorNodeId, 0]

  logger.info(
    `[ComfyUI] ReActor 换脸已注入 (model=${swapModel}, faces=${faceIndices}, detection=${faceDetection})`
  )
}
