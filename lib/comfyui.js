...
export async function generateImage(params) {
  ...
  if (params.referenceImage) {
    try {
      referenceImageName = await uploadImage(apiUrl, params.referenceImage, `ref_${Date.now()}.png`)
...
export function assemblePrompt(params) {
  ...
  if (isKrea2) {
    const qualityPrefix = config?.krea2QualityPrefix || "masterpiece, best quality, highres, newest"
