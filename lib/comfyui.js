...
export function resolveWorkflow(params) {
  ...
  if (style === "anima" || style === "anime") return config?.defaultWorkflow || "anima29_turbo"

  return config?.defaultWorkflow || "anima29_turbo"
}
...
export function assemblePrompt(params) {
  ...
  // Anima 模型：三段式提示词组装
  const qualityPrefix = config?.qualityPrefix || "masterpiece, best quality, source_anime, year_2026"
